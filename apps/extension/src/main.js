...
import {
  liSchemaKeys as _liSchemaKeys,
  liTypeMappings as _liTypeMappings
} from './schema.js'
import {
...
export const LinkedinToResumeJson = (() => {
  ...
  let _outputJsonBetaPartial = JSON.parse(
    JSON.stringify(resumeJsonTemplateBetaPartial)
  )
  /** @type {string[]} */
   ...
   */
  async function parseProfileSchemaJSON(
    instance,
    liResponse,
    endpoint = 'profileView'
  ) {
    const _this = instance
    ...
    try {
      ...
      if (allEducationCanBeCaptured) {
        const educationEntries = db.getValuesByKey(
          _liTypeMappings.education.tocKeys
        )
        educationEntries.forEach(edu => {
          parseAndPushEducation(edu, db, _this)
        })
        _this.debugConsole.log(
      ...
      document
        .querySelectorAll('span[class*="skill-category-entity"][class*="name"]')
        .forEach(skillNameElem => {
          // @ts-ignore
      ...
      resultSummary.sections.projects = _outputJsonStable.projects.length
        ? 'success'
        : 'empty'
      ...
      if (_this.debug) {
        console.group(
          `parseProfileSchemaJSON complete: ${document.location.pathname}`
        )
        console.log({
          db,
          _outputJsonLegacy,
  ...
  function LinkedinToResumeJson(OPT_debug, OPT_preferApi, OPT_getFullSkills) {
    ...
    this.getFullSkills =
      typeof OPT_getFullSkills === 'boolean' ? OPT_getFullSkills : true
    ...
    if (typeof window.require === 'function') {
      try {
        const recipeMap = window.require(
          'deco-recipes/pillar-recipes/profile/recipes'
        )
        ;['profilePositionGroups', 'fullProfile'].forEach(_key => {
          const key = /** @type {'profilePositionGroups' | 'fullProfile'} */ (
            _key
          )
          const decorationId = recipeMap[_voyagerEndpoints.dash[key].recipe]
          if (decorationId) {
            const oldPath = _voyagerEndpoints.dash[key].path
            _voyagerEndpoints.dash[key].path = _voyagerEndpoints.dash[
              key
            ].template.replace('{decorationId}', decorationId)
            this.debugConsole.log(
              `Patched voyagerEndpoint for ${key}; old = ${oldPath}, new = ${_voyagerEndpoints.dash[key].path}`
            )
          }
        })
      } catch (err) {
    ...
    this.preferDash =
      this.debug && /forceDashEndpoint=true/i.test(document.location.href)
    if (this.debug) {
  ...
  LinkedinToResumeJson.prototype.parseEmbeddedLiSchema =
    async function parseEmbeddedLiSchema() {
      const _this = this
      ...
      for (let x = 0; x < possibleBlocks.length; x++) {
        ...
        if (
          /educationView/.test(currSchemaBlock.innerHTML) &&
          /positionView/.test(currSchemaBlock.innerHTML)
        ) {
          try {
            ...
            if (schemaProfileId === desiredProfileId) {
              ...
              _this.debugConsole.log(
                `Parse from embedded schema, success = ${profileParserResult.parseSuccess}`
              )
              ...
              _this.debugConsole.log(
                `Valid schema found, but schema profile id of "${schemaProfileId}" does not match desired profile ID of "${desiredProfileId}".`
              )
            }
          } catch (e) {
            if (_this.debug) {
  ...
  LinkedinToResumeJson.prototype.parseViaInternalApiFullProfile =
    async function parseViaInternalApiFullProfile(useCache = true) {
      try {
        // Get full profile
  ...
  LinkedinToResumeJson.prototype.parseViaInternalApiFullSkills =
    async function parseViaInternalApiFullSkills() {
      try {
        const fullSkillsInfo = await this.voyagerFetch(
          _voyagerEndpoints.fullSkills
        )
  ...
  LinkedinToResumeJson.prototype.parseViaInternalApiContactInfo =
    async function parseViaInternalApiContactInfo() {
      try {
        const contactInfo = await this.voyagerFetch(
          _voyagerEndpoints.contactInfo
        )
        if (contactInfo && typeof contactInfo.data === 'object') {
          ...
          partialBasics.location.address = noNullOrUndef(
            ...
            _outputJsonLegacy.basics.location.address
          )
          partialBasics.email = noNullOrUndef(
            emailAddress,
            _outputJsonLegacy.basics.email
          )
  ...
  LinkedinToResumeJson.prototype.parseViaInternalApiBasicAboutMe =
    async function parseViaInternalApiBasicAboutMe() {
      try {
        const basicAboutMe = await this.voyagerFetch(
          _voyagerEndpoints.basicAboutMe
        )
  ...
  LinkedinToResumeJson.prototype.parseViaInternalApiAdvancedAboutMe =
    async function parseViaInternalApiAdvancedAboutMe() {
      try {
        const advancedAboutMe = await this.voyagerFetch(
  ...
  LinkedinToResumeJson.prototype.parseViaInternalApiRecommendations =
    async function parseViaInternalApiRecommendations() {
      try {
        const recommendationJson = await this.voyagerFetch(
   ...
   */
  LinkedinToResumeJson.prototype.getElementsThroughGroup =
   ...
   * @param {InternalDb} db
   */
  LinkedinToResumeJson.prototype.getWorkPositions = function getWorkPositions(
    db
  ) {
  ...
  LinkedinToResumeJson.prototype.parseViaInternalApiWork =
    async function parseViaInternalApiWork() {
      try {
        const workResponses = await this.voyagerFetchAutoPaginate(
  ...
  LinkedinToResumeJson.prototype.parseViaInternalApiEducation =
    async function parseViaInternalApiEducation() {
      try {
        ...
        const fullDashProfileObj = await this.voyagerFetch(
  ...
  LinkedinToResumeJson.prototype.parseViaInternalApiVolunteer =
  ...
  LinkedinToResumeJson.prototype.parseViaInternalApi =
    async function parseViaInternalApi(useCache = true) {
      try {
        let apiSuccessCount = 0
        let fullProfileEndpointSuccess = false

        fullProfileEndpointSuccess =
          await this.parseViaInternalApiFullProfile(useCache)
   ...
   */
  LinkedinToResumeJson.prototype.triggerAjaxLoadByScrolling =
    async function triggerAjaxLoadByScrolling(forceReScroll = false) {
   ...
   */
  LinkedinToResumeJson.prototype.getHasChangedSinceLastParse =
    function getHasChangedSinceLastParse(optLocale) {
      const localeToUse = optLocale || this.preferLocale
      const localeStayedSame =
        !localeToUse || optLocale === this.lastScannedLocale
   ...
   */
  LinkedinToResumeJson.prototype.getParsedProfile =
    async function getParsedProfile(useCache = true, optLocale = undefined) {
      const localeToUse = optLocale || this.preferLocale
      ...
      if (!localeMatchesUser || this.preferDash === true) {
        ...
        profileResponse = await this.voyagerFetch(
          _voyagerEndpoints.dash.fullProfile.path
        )
      } else {
        // use normal profileView
        profileResponse = await this.voyagerFetch(
          _voyagerEndpoints.fullProfileView
        )
      }
  ...
  LinkedinToResumeJson.prototype.tryParse = async function tryParse(optLocale) {
    ...
    const localeStayedSame =
      !localeToUse || localeToUse === _this.lastScannedLocale
    const localeMatchesUser =
      !localeToUse || localeToUse === _this.getViewersLocalLang()
    ...
    return new Promise(async resolve => {
      if (_this.parseSuccess) {
        ...
        _outputJsonBetaPartial = JSON.parse(
          JSON.stringify(resumeJsonTemplateBetaPartial)
        )
  ...
  LinkedinToResumeJson.prototype.parseAndGetRawJson =
    async function parseAndGetRawJson(version = 'stable') {
      await this.tryParse()
  ...
  /** @param {SchemaVersion} version */
  LinkedinToResumeJson.prototype.parseAndDownload =
    async function parseAndDownload(version = 'stable') {
  ...
  /** @param {SchemaVersion} version */
  LinkedinToResumeJson.prototype.parseAndShowOutput =
    async function parseAndShowOutput(version = 'stable') {
      const rawJson = await this.parseAndGetRawJson(version)
      const parsedExport = {
        raw: rawJson,
  ...
  LinkedinToResumeJson.prototype.showModal = function showModal(jsonResume) {
    ...
    if (modalWrapper) {
      ...
      modalWrapper
        .querySelector(`.${_toolPrefix}_closeButton`)
        .addEventListener('click', () => {
          _this.closeModal()
        })
      /** @type {HTMLTextAreaElement} */
      const textarea = modalWrapper.querySelector(
    ...
    /** @type {HTMLTextAreaElement} */
    const outputTextArea = modalWrapper.querySelector(
      `#${_toolPrefix}_exportTextField`
    )
  ...
  LinkedinToResumeJson.prototype.getUrlWithoutQuery =
  ...
  LinkedinToResumeJson.prototype.getViewersLocalLang = () => {
    ...
    const selectTag = document.querySelector(
      'select#globalfooter-select_language'
    )
    if (metaTag) {
   ...
   */
  LinkedinToResumeJson.prototype.getProfileUrnId =
    async function getProfileUrnId(allowFetch = true) {
      ...
      if (this.profileParseSummary && this.profileParseSummary.parseSuccess) {
        const profileDb = buildDbFromLiSchema(
          this.profileParseSummary.liResponse
        )
        this.profileUrnId =
  ...
  LinkedinToResumeJson.prototype.getDisplayPhoto =
    async function getDisplayPhoto() {
      let photoUrl = ''
      /** @type {HTMLImageElement | null} */
      const photoElem = document.querySelector(
      ...
      if (photoElem) {
        ...
        const { liResponse, profileSrc, profileInfoObj } =
          await this.getParsedProfile()
        const profileDb = buildDbFromLiSchema(liResponse)
        let pictureMeta
        if (profileSrc === 'profileView') {
          const miniProfile = profileDb.getElementByUrn(
            profileInfoObj['*miniProfile']
          )
          ...
          pictureMeta =
            profileInfoObj.profilePicture.displayImageReference.vectorImage
        }
        // @ts-ignore
        const smallestArtifact = pictureMeta.artifacts.sort(
          (a, b) => a.width - b.width
        )[0]
  ...
  LinkedinToResumeJson.prototype.generateVCard =
    async function generateVCard() {
      const profileResSummary = await this.getParsedProfile()
      const contactInfoObj = await this.voyagerFetch(
        _voyagerEndpoints.contactInfo
      )
   ...
   */
  LinkedinToResumeJson.prototype.exportVCard = async function exportVCard(
    profileResult,
    contactInfoObj
  ) {
    ...
    const contactInfo = /** @type {LiProfileContactInfoResponse['data']} */ (
      contactDb.tableOfContents
    )
    ...
    if (contactInfo.twitterHandles.length) {
      ...
      vCard.socialUrls['twitter'] =
        `https://twitter.com/${contactInfo.twitterHandles[0].name}`
    }
    ...
    if (
      profile.birthDate &&
      'day' in profile.birthDate &&
      'month' in profile.birthDate
    ) {
      ...
      if (!birthdayLi.year) {
         ...
         */
        console.warn(
          `Warning: User has a "partial" birthdate (year is omitted). This is not supported in vCard version 3 or under.`
        )
      } else {
    ...
    if (photoUrl) {
      try {
        ...
        vCard.photo.embedFromString(
          photoDataBase64.dataStr,
          photoDataBase64.mimeStr
        )
      } catch (e) {
   ...
   */
  LinkedinToResumeJson.prototype.voyagerFetchAutoPaginate =
   ...
   */
  LinkedinToResumeJson.prototype.formatVoyagerUrl =
   ...
   */
  LinkedinToResumeJson.prototype.voyagerFetch = async function voyagerFetch(
    fetchEndpoint,
    optHeaders = {}
  ) {
    const _this = this
          ...
          if (response.status !== 200) {
            ...
            response.text().then(text => {
              try {
                ...
                if (
                  !!_this.preferLocale &&
                  _this.preferLocale !== _defaultLocale
                ) {
                  _this.debugConsole.log(
